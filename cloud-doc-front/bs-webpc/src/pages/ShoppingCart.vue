  ...
  <div class="shoppingCart">
    ...
    <div class="content" v-if="carts.length>0">
      <ul>
        ...
        <!-- 购物车列表 -->
        <li class="product-list" v-for="(item,index) in carts" :key="index">
          <div class="pro-check">
          ...
          <div class="pro-name">
            <router-link
              :to="{ path: '/goods/details', query: {productID:item.skuId} }">
              {{item.title.length>21?item.title.substring(0,21):item.title}}
            </router-link>
          </div>
          ...
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.num"
              @change="handleChange($event,index,item.skuId)"
              :min="1"
              :max="item.num"
            ></el-input-number>
          </div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event,item.userId,item.skuId)"
                >确定</el-button>
              </div>
      ...
      <div class="cart-bar">
        <div class="cart-bar-left">
          ...
          <span class="cart-total">
            共
            <span class="cart-total-num">{{100}}</span> 件商品，已选择
            <span class="cart-total-num">{{mycount}}</span> 件
          </span>
        </div>
        <div class="cart-bar-right">
          ...
          <router-link :to="mycount > 0 ? '/confirmOrder' : ''">
            <div :class="mycount > 0 ? 'btn-primary' : 'btn-primary-disabled'">去结算</div>
          </router-link>
        </div>
      </div>
      <!-- 购物车底部导航条END -->
...
export default {
  ...
  methods: {
    ...
    deleteItem(e, id, productID) {
      this.$http.delete("/cart/"+productID)
        .then(res => {
            if(res.status == 200){
    ...
    getCarts(){
        ...
        if (res.status == "200") {
           ...
           var count = 0;
           var totalPrice =0;
           for(let n=0;n<cart1.length;n++){
               count=count+cart1[n].num;
               totalPrice = totalPrice+cart1[n].num*cart1[n].price;
           }
           this.mycount = count;
...
.shoppingCart .cart-empty .empty {
  ...
  margin: 65px 0 0;
  background: url(../assets/image/cart-empty.png) no-repeat 124px 0;
  color: #b0b0b0;
  overflow: hidden;
}
