<!--
 * @Descripttion:  列表组件，用于首页、全部商品页面的商品列表
 * @version: V 1.0
 * @Author: wxw
 * @Date: 2020-05-08 10:54:24
 -->
<template>
  <div id="myList" class="myList">
     <ul>
      <li v-for="(item,index) in list" :key="index">
        ...
        <router-link :to="{ path: '/goods/details', query: {productID:item.spuId} }">
          <img :src="item.images.split(',')[0]" alt />
          <h3>{{item.title.length>19?item.title.substring(0,19) : item.title}}</h3>
          <p>
            <span>
              <span style="border:1px green solid;color:green;" v-if="item.stock >0">有货</span>
              <span style="border:1px gray solid;color:red;" v-else>暂无售货</span>
                {{item.price}}元
            </span>
            <span v-show="item.product_price != item.product_selling_price" class="del">{{item.price}}元</span>
          </p>
        </router-link>
      </li>
